...
pub fn create_context(
    ...
    let AddressKinds {
        ...
        tls,
        local_tls,
    } = compiler.address_kinds(cg_flags);

    let mut context = CompilerContext {
        mode: compiler.machine_mode(),
        properties: TargetInfo {
            properties: TargetProperties {
                global_properties: target
                    .compile_default_properties(mach)
                    .into_iter()
                    .map(|(k, v)| {
                        let key = Symbol::intern(k.as_ref());

                        let value = match v {
                            lccc_targets::properties::ExtPropertyValue::String(val) => {
                                PropertyValue::String(Symbol::intern(val.as_ref()))
                            }
                            lccc_targets::properties::ExtPropertyValue::Bool(val) => {
                                PropertyValue::Bool(val)
                            }
                            lccc_targets::properties::ExtPropertyValue::Int(val) => {
        ...
        target_features: target
            .compile_target_features(mach)
            .into_iter()
            .map(|v| v.to_box().into_string())
            .collect(),
