...
use cmli::{
    archs::skyarch::{
        Map, Skyarch, SkyarchCoprocessor, SkyarchRegRangeIter, SkyarchRegister, SkyarchRegno,
    },
    as_id_array,
    mach::{MachineMode, OneMachine, Register},
    skyarch_regno,
    traits::IdType,
};
...
impl CallConvSpec for SkyarchCc {
        ...
        Self: Sized,
    {
        match name {
            "C" => Some(SkyarchCc {
                use_hardfp: None,
                use_hardvreg: None,
                ireg_range: RangeInclusive {
                    start: skyarch_regno!(1),
                    last: skyarch_regno!(10),
                },
                iret_regs: RangeInclusive {
                    start: skyarch_regno!(1),
                    last: skyarch_regno!(4),
                },
            }),
            _ => None,
        }
    ...
    fn replace_with_memory_param(
        ...
        if frags
            .iter()
            .any(|(a, _)| *a == ParameterFragmentClass::Memory)
        {
            return Some((ParameterFragmentClass::Integer, 4));
        }
